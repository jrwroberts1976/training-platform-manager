...
def sync_course(course: Course, settings: Settings) -> None:
    ...
    if (destination / ".git").exists():
        ...
        run(["git","checkout",course.branch], cwd=destination)
        run(["git","reset","--hard",f"origin/{course.branch}"], cwd=destination)
        run(["git","clean","-fd"], cwd=destination)
        return
    if destination.exists():
        raise RuntimeError(f"{destination} exists but is not a normal Git repository")
    destination.parent.mkdir(parents=True, exist_ok=True)
    run(["git","clone","--branch",course.branch,"--single-branch",course.repository,str(destination)])
