...
def course_navigation(course: Course, settings: Settings) -> list[str]:
    ...
    for module in sorted((p for p in modules_root.iterdir() if p.is_dir()), key=numeric_key):
        ...
        if readme.exists():
            lines.append(f"              - Overview: courses/{course.slug}/modules/{module.name}/README.md")
        for lesson in sorted((p for p in module.glob("*.md") if p.name != "README.md"), key=numeric_key):
            lines.append(f"              - {yaml_quote(markdown_title(lesson))}: courses/{course.slug}/modules/{module.name}/{lesson.name}")
    return lines
...
def update_mkdocs(courses: list[Course], settings: Settings) -> None:
    ...
    lines = [settings.start_marker]
    for course in courses:
        if course.enabled:
            lines.extend(course_navigation(course, settings))
    lines.append(settings.end_marker)
    settings.mkdocs_file.write_text(before + "\n".join(lines) + after, encoding="utf-8")
